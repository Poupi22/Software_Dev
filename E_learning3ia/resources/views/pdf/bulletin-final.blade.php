...
<head>
    ...
    <title>Relevé de Notes Final</title>
    <style>
        body::before {
            content: "";
            position: fixed;
        ...
        .tete { width: 100%; border-collapse: collapse; table-layout: fixed; border: none; }
        .tete td { width: 33.33%; text-align: center; vertical-align: middle; padding: 2px; font-size: 10px; word-wrap: break-word; border: none; }
        .tete .content { text-align: center; line-height: 1.1; }
        .tete .content span { display: block; font-size: 0.9em; white-space: nowrap; margin: 0; padding: 0; }
        ...
        .inner-details span { display: block; font-size: 12px; margin: 5px; padding: 0; line-height: 1.2; white-space: normal; text-transform: uppercase; }
        ...
        /* Ligne séparatrice de semestre */
        tr.semestre-header td {
        ...
        /* Ligne sous-total semestre */
        tr.semestre-bilan td {
        ...
        .ligne-signature { margin-top: 38px; border-top: 1px solid #555; width: 80%; margin-left: auto; margin-right: auto; }
...
<table>
    ...
    <tbody>
        @foreach ($bulletinData['bulletins_semestres'] as $numSemestre => $bulletinSemestre)

            {{-- Séparateur de semestre --}}
            <tr class="semestre-header">
                <td colspan="7"> SEMESTRE {{ $numSemestre }}</td>
            </tr>
            ...
            {{-- Sous-total semestre --}}
            <tr class="semestre-bilan">
                <td colspan="2" style="text-align:left; padding-left:4px;">
                    Bilan S{{ $numSemestre }} &mdash; Mention : <strong>{{ $bulletinSemestre['mention'] }}</strong>
                </td>
                <td>
                    @php
                        $creditsS = collect($bulletinSemestre['notes'])
                            ->filter(fn($n) => $n['note_finale'] !== null && $n['note_finale'] >= 10)
                            ->sum('credit');
                    @endphp
                    Crédits : <strong>{{ $creditsS }}</strong>
                </td>
                <td colspan="4">
                    Moyenne S{{ $numSemestre }} :
                    <strong>
                        {{ $bulletinSemestre['moyenne_generale'] !== null
                            ? number_format($bulletinSemestre['moyenne_generale'], 2) . '/20'
                            : '—' }}
                    </strong>
                    &nbsp;|&nbsp;
                    Assiduité :
                    <strong>{{ $bulletinSemestre['assiduite']['presence'] ?? 0 }}%</strong>
                </td>
            </tr>
        ...
        {{-- BILAN GÉNÉRAL FINAL --}}
        <tr class="bilan-general">
            <td colspan="2" style="text-align:left; padding-left:4px;">
                Mention Finale : <strong>{{ $bulletinData['mention_finale'] }}</strong>
            </td>
            <td>
                @php
                    $totalCreditsCapitalises = 0;
                    foreach ($bulletinData['bulletins_semestres'] as $bs) {
                        $totalCreditsCapitalises += collect($bs['notes'])
                            ->filter(fn($n) => $n['note_finale'] !== null && $n['note_finale'] >= 10)
                            ->sum('credit');
                    }
                @endphp
                Total Crédits : <strong>{{ $totalCreditsCapitalises }}</strong>
            </td>
            <td colspan="4">
                Moyenne Annuelle :
                <strong>
                    {{ $bulletinData['moyenne_annuelle'] !== null
                        ? number_format($bulletinData['moyenne_annuelle'], 2) . '/20'
                        : '—' }}
                </strong>
                &nbsp;|&nbsp;
                Résultat :
                <strong>{{ ($bulletinData['moyenne_annuelle'] ?? 0) >= 10 ? 'ADMIS(E)' : 'AJOURNÉ(E)' }}</strong>
            </td>
        </tr>
    </tbody>
</table>
