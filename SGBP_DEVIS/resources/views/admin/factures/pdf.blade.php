...
{{-- ══════ FILIGRANE ══════ --}}
@if($facture->type === 'provisoire')
    <div class="watermark">PROVISOIRE</div>
@else
    <div class="watermark">DÉFINITIVE</div>
@endif
...
<div class="header-wrap">
    <div class="header-left">
        <div class="doc-type">
            Facture {{ $facture->type === 'provisoire' ? 'Provisoire' : 'Définitive' }}
        </div>
        <div class="doc-numero">N° {{ $facture->numero }}</div>
        <div class="doc-date">{{ $facture->date_emission->translatedFormat('d F Y') }}</div>
    </div>
...
@if($facture->titre)
<div class="doc-titre">Objet : {{ $facture->titre }}</div>
@endif
...
<div class="parties-wrap">
    ...
    <div class="party-col">
        ...
        @if($facture->client->type === 'societe')
            {{-- CLIENT SOCIÉTÉ --}}
            <div class="party-row">
                <div class="party-label">Société :</div>
                <div class="party-value bold">{{ $facture->client->raison_sociale ?? $facture->client->nom_complet }}</div>
            </div>
            @if($facture->client->nif ?? null)
            <div class="party-row">
                <div class="party-label">NIF :</div>
                <div class="party-value">{{ $facture->client->nif }}</div>
            </div>
            @endif
            @if($facture->client->pays ?? null)
            <div class="party-row">
                <div class="party-label">Pays :</div>
                <div class="party-value">{{ $facture->client->pays }}</div>
            </div>
            @endif
            @if($facture->client->ville ?? null)
            <div class="party-row">
                <div class="party-label">Ville :</div>
                <div class="party-value">{{ $facture->client->ville }}</div>
            </div>
            @endif
            @if($facture->client->adresse ?? null)
            <div class="party-row">
                <div class="party-label">Adresse :</div>
                <div class="party-value">{{ $facture->client->adresse }}</div>
            </div>
            @endif
            @if($facture->client->bp ?? null)
            <div class="party-row">
                <div class="party-label">BP :</div>
                <div class="party-value">{{ $facture->client->bp }}</div>
            </div>
            @endif
            @if($facture->client->telephone_principal ?? null)
            <div class="party-row">
                <div class="party-label">Téléphone :</div>
                <div class="party-value">{{ $facture->client->telephone_principal }}</div>
            </div>
            @endif
            @if($facture->client->email ?? null)
            <div class="party-row">
                <div class="party-label">Email :</div>
                <div class="party-value">{{ $facture->client->email }}</div>
            </div>
            @endif
        ...
        @else
            ...
            <div class="party-row">
                ...
                <div class="party-value bold">{{ $facture->client->nom_complet }}</div>
            </div>
            @if($facture->client->pays ?? null)
            <div class="party-row">
                <div class="party-label">Pays :</div>
...
{{-- ══════ INTRODUCTION ══════ --}}
@if($facture->introduction)
    <div class="intro-block">{!! nl2br(e($facture->introduction)) !!}</div>
@endif
    ...
    <tbody>
        ...
        @foreach($facture->categories as $cat)
            ...
            <tr>
                ...
                <td class="text-right">{{ number_format($art->prix_unitaire_ht, 2, ',', ' ') }} {{ $facture->devise }}</td>
                <td class="text-center">{{ $art->quantite }}</td>
                <td class="text-center">
                    {{ $art->remise_pourcentage > 0 ? number_format($art->remise_pourcentage, 0).'%' : '' }}
                </td>
                <td class="text-right">{{ number_format($art->total_ht, 2, ',', ' ') }} {{ $facture->devise }}</td>
            </tr>
            @endforeach
            ...
            <tr class="mo-row">
                ...
                <td class="text-right">{{ number_format($cat->main_oeuvre, 2, ',', ' ') }} {{ $facture->devise }}</td>
        ...
        @if($sansCategorie->count() > 0)
            @if($facture->categories->count() > 0)
            <tr class="cat-row"><td colspan="6">Autres articles</td></tr>
            @endif
        ...
        @if($facture->main_oeuvre > 0)
        <tr class="mo-row">
            <td class="text-center">Service</td>
            <td>Main d'Œuvre</td>
            <td class="text-right">{{ number_format($facture->main_oeuvre, 2, ',', ' ') }} {{ $facture->devise }}</td>
            <td class="text-center">1</td>
            <td></td>
            <td class="text-right">{{ number_format($facture->main_oeuvre, 2, ',', ' ') }} {{ $facture->devise }}</td>
        </tr>
...
{{-- ══════ TOTAUX ══════ --}}
@php
    $totalMainOeuvre = $facture->main_oeuvre + $facture->categories->sum('main_oeuvre');
@endphp
<div class="totals-wrap">
    <div class="totals-left">
        <div class="montant-lettres">
            Arrêté la présente facture à la somme de :<br>
            <span>{{ \App\Helpers\NombreEnLettres::convertir($facture->total_ttc, $facture->devise) }}</span>
        </div>
    </div>
    <div class="totals-right">
        <table class="totals-table">
            <tr>
                ...
                <td class="tot-value">{{ number_format($facture->total_ht, 2, ',', ' ') }} {{ $facture->devise }}</td>
            </tr>
            @if($totalMainOeuvre > 0)
            <tr>
                <td class="tot-label">Main d'Œuvre (HT)</td>
                <td class="tot-value">{{ number_format($totalMainOeuvre, 2, ',', ' ') }} {{ $facture->devise }}</td>
            </tr>
            <tr>
                <td class="tot-label" style="color:#059669;">Sous-total HT</td>
                <td class="tot-value" style="color:#059669;">{{ number_format($facture->total_ht + $totalMainOeuvre, 2, ',', ' ') }} {{ $facture->devise }}</td>
            </tr>
            @endif
            <tr>
                <td class="tot-label">TPS {{ number_format($facture->taux_tps, 2) }}% (sur M.O.)</td>
                <td class="tot-value">{{ number_format($facture->total_tps, 2, ',', ' ') }} {{ $facture->devise }}</td>
            </tr>
            <tr>
                <td class="tot-label">CSS {{ number_format($facture->taux_css, 2) }}% (sur HT)</td>
                <td class="tot-value">{{ number_format($facture->total_css, 2, ',', ' ') }} {{ $facture->devise }}</td>
            </tr>
            <tr class="net-row">
                <td>Total TTC</td>
                <td>{{ number_format($facture->total_ttc, 2, ',', ' ') }} {{ $facture->devise }}</td>
            </tr>
        </table>
    </div>
</div>
...
<div class="conditions-section">
    <div class="conditions-title">État du paiement</div>
    <div class="conditions-row">
        <div class="conditions-label">Montant payé :</div>
        <div class="conditions-value">{{ number_format($facture->montant_paye, 2, ',', ' ') }} {{ $facture->devise }}</div>
    </div>
    <div class="conditions-row">
        <div class="conditions-label">Reste à payer :</div>
        <div class="conditions-value">{{ number_format($facture->reste_a_payer, 2, ',', ' ') }} {{ $facture->devise }}</div>
    </div>
</div>
@endif
...
<div class="conditions-section">
    ...
    <div class="conditions-row">
        ...
        <div class="conditions-value">{{ $facture->conditions_paiement }}</div>
    </div>
    @endif
    <div class="conditions-row">
        <div class="conditions-label">Date d'échéance :</div>
        <div class="conditions-value">{{ $facture->date_echeance->translatedFormat('d F Y') }}</div>
    </div>
    <div class="conditions-row">
        <div class="conditions-label">Devise :</div>
        <div class="conditions-value">{{ $facture->devise }}</div>
    </div>
</div>

{{-- ══════ CONCLUSION ══════ --}}
@if($facture->conclusion)
    <div class="conclusion-block" style="margin-top:8px;">{!! nl2br(e($facture->conclusion)) !!}</div>
@endif
...
<div class="footer">
    <div class="footer-inner">
        ...
        <div class="footer-right">
            Facture N° {{ $facture->numero }} — Page <span class="pagenum"></span>
        </div>
    </div>
</div>
