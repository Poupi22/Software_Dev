    ...
    <div class="min-h-screen">
        <div class="bg-white border-b border-gray-200 px-4 md:px-8 py-3 md:py-4 sticky top-0 z-10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-xl md:text-2xl font-bold text-gray-800">Modifier : {{ $service->nom }}</h2>
                    <p class="text-xs text-gray-500">Modifier les informations du service</p>
                </div>
            </div>
        </div>
            ...
            <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data"
                ...
                <div>
                    ...
                    <input type="text" name="description_courte"
                        value="{{ old('description_courte', $service->description_courte) }}"
                        class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:border-blue-500"
                ...
                <div>
                    ...
                    <textarea name="description" rows="5" required
                        class="w-full px-4 py-3 border rounded-lg focus:outline-none focus:border-blue-500 resize-none">{{ old('description', $service->description) }}</textarea>
                </div>
